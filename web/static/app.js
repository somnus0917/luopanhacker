...
function collectedPanelSection() {
  ...
  if (state.douyinSection === "product_card") {
    const products = selected.flatMap((panel) => (panel.products || []).map((product) => ({ ...product, shop_name: panel.shop_name })));
    ...
    return `${snapshotBanner(date, "商品卡", "已验证：近 1 天 → 商品卡列表")}${metricCards([
      ...
      ["商品点击人数", whole(clicks), `点击率 ${ratio(exposure ? clicks / exposure : 0)}`],
      ["点击至成交", ratio(clicks ? buyers / clicks : 0), "成交人数 ÷ 商品点击人数"]
    ])}<section class="panel"><div class="panel-head"><div><h3>商品卡表现</h3><span>按支付金额排序 · 最多展示 30 个商品</span></div><span class="chart-semantic">商品转化</span></div>${compactTable(["店铺", "商品", "支付金额", "曝光", "点击", "点击率", "点击成交率"], rows2, "该日期未返回商品卡明细")}</section>${verification}`;
  }
  const isLive = state.douyinSection === "live";
  const metricSpec = isLive ? [["直播成交金额", "pay_amt", "money", "直播数据概览"], ["成交订单", "pay_cnt", "whole", "直播间支付订单"], ["观看人数", "watch_cnt", "whole", "直播间累计观看"], ["开播场次", "room_cnt", "whole", "昨日开播直播间"]] : [["短视频成交金额", "pay_amt", "money", "短视频数据概览"], ["成交订单", "pay_cnt", "whole", "短视频支付订单"], ["商品曝光次数", "product_show_cnt", "whole", "短视频带货商品曝光"], ["引流店铺成交", "lead_shop_pay_amt", "money", "短视频引流店铺页"]];
