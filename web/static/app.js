...
function inventoryWarehouseOptions(payload) {
  return [
    ...new Set(
      (payload.rows || []).map(
        (item) => item.warehouse_name || "未命名仓库"
      )
    )
  ...
  return INVENTORY_HEALTH_ORDER.map((key) => {
    ...
    return {
      ...
      available_num: members.reduce(
        (sum2, row) => sum2 + number(row.available_num),
        0
      )
    };
  });
...
function inventorySummary(rows) {
  const coverageRows = rows.filter(
    (row) => row.coverage_days !== null && row.coverage_days !== void 0 && number(row.sales_7d) > 0
  );
  ...
  return {
    ...
    salable_skus: new Set(
      rows.filter((row) => number(row.available_num) > 0).map((row) => row.spec_no).filter(Boolean)
    ).size,
    ...
    replenishment_records: rows.filter(
      (row) => ["out_of_stock", "urgent", "replenish"].includes(row.health_key)
    ).length,
    no_movement_records: rows.filter((row) => row.health_key === "no_movement").length,
    overstock_records: rows.filter(
      (row) => ["overstock", "high"].includes(row.health_key)
    ).length,
    ...
    available_cost_amount: costRows.length ? costRows.reduce(
      (sum2, row) => sum2 + number(row.available_cost_amount),
      0
    ) : null
  };
}
...
function inventoryTable(rows, mode = "detail") {
  ...
  const body = visible.map((item) => {
    ...
    const cells = mode === "replenish" ? shared : [
      healthPill(item),
      item.warehouse_name,
      item.brand_name,
      item.goods_name,
  ...
  const headerCells = columnConfigs.map(
    (col) => `<th ${col.key ? `data-sort-key="${col.key}"` : ""} class="${state.inventorySortKey === col.key ? "sorted-" + state.inventorySortDir : ""}">${col.label}</th>`
  ).join("");
...
function healthDistribution(items) {
  ...
  const cards = items.map(
    (item) => `<div class="health-row"><div class="health-row-head">${healthPill(item)}<strong>${whole(item.sku_records)} 条</strong></div><div class="health-track"><i class="${escapeHtml(item.key)}" style="width:${Math.max(2, number(item.sku_records) / max * 100)}%"></i></div><span>可发 ${whole(item.available_num)}</span></div>`
  ).join("");
...
function salesTrendPanel(items) {
  ...
  const bars = items.map(
    (item) => `<div class="sales-day"><span>${escapeHtml(String(item.date || "").slice(5) || "—")}</span><div class="sales-column"><i style="height:${Math.max(4, number(item.quantity) / max * 100)}%"></i></div><strong>${whole(item.quantity)}</strong></div>`
  ).join("");
...
function douyinAvailableDates() {
  ...
  return [...new Set(source.map((record) => String(record.date || "")))].filter(Boolean).sort((left, right) => right.localeCompare(left));
}
function isDouyinDateSelected(date) {
...
function douyinCalendarDate(year, month, day) {
  return `${year}-${String(month).padStart(2, "0")}-${String(day).padStart(2, "0")}`;
}
function douyinCalendarMonthStart(value) {
  const [year, month] = value.split("-").map(Number);
  return new Date(Date.UTC(year, month - 1, 1));
}
function shiftDouyinCalendarMonth(value, amount) {
  const cursor = douyinCalendarMonthStart(value);
  cursor.setUTCMonth(cursor.getUTCMonth() + amount);
  return douyinCalendarDate(cursor.getUTCFullYear(), cursor.getUTCMonth() + 1, 1);
}
function selectDouyinDatePreset(preset) {
  ...
  if (preset === "all") {
    ...
    const earliest = /* @__PURE__ */ new Date(`${latest}T00:00:00Z`);
...
function douyinCalendarMonthMarkup(monthValue, availableDates, rangeStart, rangeEnd) {
  const cursor = douyinCalendarMonthStart(monthValue);
  const year = cursor.getUTCFullYear();
  const month = cursor.getUTCMonth() + 1;
  const firstWeekday = cursor.getUTCDay() || 7;
  const gridStart = new Date(Date.UTC(year, month - 1, 2 - firstWeekday));
  const today = douyinCalendarDate((/* @__PURE__ */ new Date()).getFullYear(), (/* @__PURE__ */ new Date()).getMonth() + 1, (/* @__PURE__ */ new Date()).getDate());
  const days = Array.from({ length: 42 }, (_, index) => {
    const date = new Date(gridStart);
    date.setUTCDate(gridStart.getUTCDate() + index);
...
function douyinCalendarMarkup() {
  ...
  const presets = [["day", "近 1 日"], ["week", "近 7 日"], ["month", "近 30 天"], ["custom", "自定义日期"]];
...
function douyinFiltersMarkup() {
  ...
  const tags = [douyinDateLabel(), state.douyinShop ? `店铺：${state.douyinShop}` : "全部店铺"].map((tag) => `<span class="filter-summary-tag">${escapeHtml(tag)}</span>`).join("");
...
function snapshotBanner(label, detail) {
...
function productCardSection(records) {
  const scoped = records;
  const channelRecords = (state.channel?.records || []).filter((record) => isDouyinDateSelected(String(record.date || "")) && (!state.douyinShop || record.shop_name === state.douyinShop));
...
function collectedPanelSection() {
  ...
  const panels = all.flatMap((record) => (record.panels || []).map((panel) => ({ ...panel, date: record.date, shop_name: record.shop_name })));
  ...
  const panelMetric = (panel, key) => number(panel.metrics?.[key]);
  const panelMetricTotal = (key) => selected.reduce((total, panel) => total + panelMetric(panel, key), 0);
  ...
  if (state.douyinSection === "product_card") {
    const products = selected.flatMap((panel) => (panel.products || []).map((product) => ({ ...product, date: panel.date, shop_name: panel.shop_name })));
    ...
    const rows2 = products.sort((left, right) => number(right.pay_amt) - number(left.pay_amt)).slice(0, 30).map((product) => [
    ...
    return `${snapshotBanner("商品卡", "按所选数据日期汇总")}${metricCards([
      ...
      ["商品点击人数", whole(clicks), `点击率 ${ratio(exposure ? clicks / exposure : 0)}`],
      ["点击至成交", ratio(clicks ? buyers / clicks : 0), "成交人数 ÷ 商品点击人数"]
    ])}<section class="panel"><div class="panel-head"><div><h3>商品卡表现</h3><span>按支付金额排序 · 最多展示 30 个商品</span></div><span class="chart-semantic">商品转化</span></div>${compactTable(["日期", "店铺", "商品", "支付金额", "曝光", "点击", "点击率", "点击成交率"], rows2, "所选日期未返回商品卡明细")}</section>${verification}`;
  }
  const isLive = state.douyinSection === "live";
  const metricSpec = isLive ? [["直播成交金额", "pay_amt", "money", "直播数据概览"], ["成交订单", "pay_cnt", "whole", "直播间支付订单"], ["观看人数", "watch_cnt", "whole", "直播间累计观看"], ["开播场次", "room_cnt", "whole", "所选日期开播直播间"]] : [["短视频成交金额", "pay_amt", "money", "短视频数据概览"], ["成交订单", "pay_cnt", "whole", "短视频支付订单"], ["商品曝光次数", "product_show_cnt", "whole", "短视频带货商品曝光"], ["引流店铺成交", "lead_shop_pay_amt", "money", "短视频引流店铺页"]];
  const typedMetricSpec = metricSpec;
  const metricValue = (key, kind) => kind === "money" ? money(panelMetricTotal(key)) : whole(panelMetricTotal(key));
...
function renderDouyin() {
  ...
  const content = state.douyin?.records?.length ? collectedPanelSection() : !records.length ? `<div class="empty-panel"><strong>所选日期暂无抖音日快照</strong><span>请切换其他日期范围；若没有可选日期，请等待采集器写入首个快照。</span></div>` : state.douyinSection === "product_card" ? productCardSection(records) : contentSection(records, state.douyinSection, state.douyinSection === "live" ? "直播" : "短视频");
  ...
  $$("#douyin-content [data-douyin-date-preset]").forEach((button) => button.addEventListener("click", () => {
    const preset = button.dataset.douyinDatePreset;
    if (preset === "custom") {
