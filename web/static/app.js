...
function operationFilterItems(kind, records = state.records) {
    ...
    if (kind === "date")
        return [...new Set([...records.map((item) => item.date), ...channelRecords.map((item) => item.date)])].sort();
    if (kind === "platform")
        return [...new Set([...records.map(recordPlatform), ...(channelRecords.length ? ["抖音"] : [])])].sort((a, b) => a.localeCompare(b, "zh-CN"));
    if (kind === "shop")
        return [...new Set([...records.map((item) => item.shop_name), ...channelRecords.map((item) => item.shop_name)])].sort((a, b) => a.localeCompare(b, "zh-CN"));
