    ...
    result.update(
        {
            ...
            "actual_turnover_days": rounded(
                average_available / (sales_quantity / ACTUAL_TURNOVER_WINDOW_DAYS)
            )
            if sales_quantity
            else None,
        }
...
def health_for(
    available_num: float, sales_7d: float
    ...
    coverage_days = (
        math.ceil(max(available_num, 0) / daily_sales) if daily_sales else None
    )
    replenish_qty = (
        math.ceil(
            max(
                0,
                (TARGET_COVER_DAYS + SAFETY_STOCK_DAYS) * daily_sales
                - max(available_num, 0),
            )
        )
        if daily_sales
...
def load_inventory_dashboard() -> dict[str, Any] | None:
    ...
    for row in snapshot.get("sales_7d", []):
        sales[(row.get("warehouse_no", ""), row.get("spec_no", ""))] += number(
            row.get("quantity")
        )
    inbound = defaultdict(float)
    for row in snapshot.get("inbound_30d", []):
        inbound[(row.get("warehouse_no", ""), row.get("spec_no", ""))] += number(
            row.get("quantity")
        )
    ...
    warehouse_names = {
        str(row.get("warehouse_no") or ""): str(
            row.get("warehouse_name") or row.get("warehouse_no") or "未命名仓库"
        )
        for row in rows
    }
    ...
    def build_group(key_name: str) -> list[dict[str, Any]]:
        ...
        for group in groups.values():
            group["turnover_days"] = (
                rounded(group["available_num"] / (group["sales_7d"] / 7))
                if group["sales_7d"]
                else None
            )
            for metric in ("stock_num", "available_num", "sales_7d", "inbound_30d"):
    ...
    total_sales_7d = sum(
        number(row.get("quantity")) for row in snapshot.get("sales_7d", [])
    )
    health_order = [
        "out_of_stock",
        "urgent",
        ...
        health.append(
            {
                ...
                "available_num": rounded(
                    sum(number(row.get("available_num")) for row in members)
                ),
            }
        )

    sales_by_date = defaultdict(float)
    sales_by_warehouse_date: dict[str, defaultdict[str, float]] = defaultdict(
    ...
    for row in snapshot.get("sales_7d", []):
        if row.get("date"):
            ...
            warehouse_name = warehouse_names.get(
                str(row.get("warehouse_no") or ""), "未命名仓库"
            )
            sales_by_warehouse_date[warehouse_name][row["date"]] += quantity
    sales_trend_7d = [
        {"date": date, "quantity": rounded(quantity)}
        for date, quantity in sorted(sales_by_date.items())
    ]
    sales_trend_7d_by_warehouse = {
        warehouse: [
            {"date": date, "quantity": rounded(quantity)}
            for date, quantity in sorted(values.items())
        ]
    ...
    coverage_rows = [
        row["coverage_days"]
        for row in rows
        if row["coverage_days"] is not None and row["sales_7d"] > 0
    ]
    summary = {
        "sku_records": len(rows),
        "distinct_skus": len(
            {row.get("spec_no") for row in rows if row.get("spec_no")}
        ),
        "salable_skus": len(
            {row.get("spec_no") for row in rows if number(row.get("available_num")) > 0}
        ),
        ...
        "inbound_30d": rounded(
            sum(number(row.get("quantity")) for row in snapshot.get("inbound_30d", []))
        ),
        "negative_available": sum(number(row.get("available_num")) < 0 for row in rows),
        "turnover_days": rounded(total_available / (total_sales_7d / 7))
        if total_sales_7d
        else None,
        "average_coverage_days": rounded(sum(coverage_rows) / len(coverage_rows))
        if coverage_rows
        else None,
        "replenishment_records": sum(
            row["health_key"] in {"out_of_stock", "urgent", "replenish"} for row in rows
        ),
        "no_movement_records": sum(row["health_key"] == "no_movement" for row in rows),
        "overstock_records": sum(
            row["health_key"] in {"overstock", "high"} for row in rows
        ),
    }
    priority = {key: index for index, key in enumerate(health_order)}
    rows.sort(
        key=lambda row: (
            priority[row["health_key"]],
            number(row.get("coverage_days") or 10**9),
            -number(row.get("sales_7d")),
        )
    )
