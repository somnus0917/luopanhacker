...
SENSITIVE_KEY_PARTS = ("token", "verify", "bogus", "cookie", "secret", "password", "signature")
ENDPOINTS = (
...
def is_business_response(url):
    ...
    return trusted_host and any(
        parsed.path.startswith(prefix) for prefix in ENDPOINTS
    )
...
def sanitized_url(url):
    ...
    for key, value in parse_qsl(parsed.query, keep_blank_values=True):
        lowered = key.lower()
        if "token" in lowered or "verify" in lowered or "bogus" in lowered or lowered in {"fp", "_lid"}:
            continue
...
def sensitive_key(key):
    ...
    return lowered in {"fp", "_lid"} or any(part in lowered for part in SENSITIVE_KEY_PARTS)


def sanitized_payload(value):
    if isinstance(value, dict):
        return {key: sanitized_payload(item) for key, item in value.items() if not sensitive_key(key)}
...
def sanitized_post_data(raw):
    if not raw:
        return None
    try:
        return json.dumps(sanitized_payload(json.loads(raw)), ensure_ascii=False, separators=(",", ":"))
    except json.JSONDecodeError:
        fields = [(key, value) for key, value in parse_qsl(raw, keep_blank_values=True) if not sensitive_key(key)]
...
class ResponseCapture:
    ...
    async def _capture(self, response, shop_name):
        ...
        try:
            ...
            if body_size > MAX_BODY_BYTES or self.body_bytes + body_size > MAX_TOTAL_BODY_BYTES:
                print(f"渠道响应过大，已跳过: {urlparse(response.url).path}", flush=True)
                return
...
async def load(page):
    ...
    for module_index, tablist in enumerate(date_tablists[1:], start=1):
        ...
        targets = [candidates.nth(index) for index in range(await candidates.count()) if await candidates.nth(index).is_visible()]
        if len(targets) != 1:
        ...
        try:
            ...
            print(f"渠道模块 {module_index} 切换近1天失败，保留其他数据: {exc!r}", flush=True)
