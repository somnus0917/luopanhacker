...
def date_range_from_fields(fields, required_date_type=None):
    if required_date_type is not None and str(fields.get("date_type", [None])[0]) != required_date_type:
        return None
...
def date_range_from_request(url, post_data, required_date_type=None):
    date_range = date_range_from_fields(parse_qs(urlparse(url).query), required_date_type)
...
async def visible_custom_day(page, data_day):
    ...
    for selector in selectors:
        ...
        for index in range(await locator.count()):
            ...
            try:
                if not await candidate.is_visible() or await candidate.get_attribute("aria-disabled") == "true":
                    continue
            ...
            if box is None:
                continue
            identity = tuple(round(box[key], 1) for key in ("x", "y", "width", "height"))
    ...
    panels = page.locator('[class*="picker-panel"], [class*="calendar-panel"], [role="dialog"]')
    ...
    for index in range(await panels.count()):
        ...
        try:
            ...
            continue
        if not any(marker in text for marker in year_markers) or not any(marker in text for marker in month_markers):
            continue
...
async def choose_data_day(page, data_day=None):
    ...
    if len(date_ranges) != 1:
        raise RuntimeError(f"未能确认“{label}”的实际数据日期: {sorted(date_ranges)!r}; 接口: {sorted(core_urls)!r}")
    result = date_ranges.pop()
    if expected is not None and result != expected:
        raise RuntimeError(f"“{label}”返回日期 {result!r}，与目标日期 {expected!r} 不一致")
...
async def run(args):
    ...
    if data_day is not None and "channel" in requested:
        raise ValueError("指定日期补采原型暂仅支持 --module operations；渠道子模块仍需独立日期选择验证")
...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(description="罗盘独立采集服务：共享登录会话，采集昨日或指定历史日期数据")
    ...
    parser.add_argument("--date", type=parse_data_day, help="指定本月历史日期，格式 YYYY-MM-DD；暂仅支持经营模块")
